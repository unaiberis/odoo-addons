...
class ProductTemplate(models.Model):
    ...
    price_cost_changed = fields.Boolean(
        string="Price/Cost Changed", default=False)
    sale_configuration = fields.Boolean(
        string='Sale conf: Base + Attributes', default=False)
    standard_price = fields.Float(
        string='Total cost (base + extra)', compute=False,
        inverse=False, search=False, store=True, copy=False)
    ...
    base_cost = fields.Float(
        string='Base cost', copy=False, default=0.0)
    extra_cost = fields.Float(
        string='Extra cost', copy=False, default=0.0)
    target_cost = fields.Float(
        string='Target cost', copy=False, default=0.0)
    ...
    last_price_change_date = fields.Date(
        string='Last price change date', copy=False)
    ...
    separator_1 = fields.Char(
        string="||", default="||")
    new_extra_cost = fields.Float(
        string='New extra cost', copy=False, default=0.0)
    last_change_date_new_extra_cost = fields.Date(
        string='Last change date new extra cost', readonly=True, copy=False)
    new_target_cost = fields.Float(
        string='New target cost', copy=False, default=0.0)
    ...
    separator_2 = fields.Char(
        string="||", default="||")
    percentage_between_costs = fields.Float(
        string="Percentage between costs", digits=(16, 2),
        compute="_compute_percentage_between_costs", copy=False, store=True)
    ...
    @api.onchange('manual_pvp', 'target_cost', 'new_target_cost',
    ...
    def _onchange_category_sale_price(self):
        ...
        if self.env.user.id not in group.users.ids:
            raise ValidationError(
                _('You do not have permission to change the sale price of the'
                  ' product.'))
        for product in self:
            product.price_cost_changed = True
            if ('change_manual_pvp' in self.env.context or
                    'change_new_target_data' not in self.env.context):
                with_lst_price = True if product.list_price else False
            if ('change_manual_pvp' in self.env.context or
                    'change_new_target_data' in self.env.context):
                with_new_sale_price = True if product.new_sale_price else False
            imp = 0
            new_sale_price = 0
            if ('change_manual_pvp' in self.env.context or
                    'change_new_target_data' not in self.env.context):
                if (not product.manual_pvp and
                        product.product_category_sale_price_id):
                    category = product.product_category_sale_price_id
                    imp = (
                        product.target_cost + (
                            (product.target_cost * category.percentage) /
                            100) + category.fixed_amount)
            if ('change_manual_pvp' in self.env.context or
                    'change_new_target_data' in self.env.context):
                if (not product.manual_pvp and
                    ...
                    new_sale_price = (
                        product.new_target_cost + (
                            (product.new_target_cost * category.percentage) /
                            100) + category.fixed_amount)
            if ('change_manual_pvp' in self.env.context or
                    'change_new_target_data' in self.env.context):
            ...
            title = False
            if (('change_manual_pvp' in self.env.context or
                 'change_new_target_data' not in self.env.context) and
                    (with_lst_price and imp == 0 and product.name)):
                title = _('Warning for product: %s') % product.name
                message = _('You just changed the sale price')
                warning = {'title': title,
                           'message': message}
            if (('change_manual_pvp' in self.env.context or
                 'change_new_target_data' in self.env.context) and
                (with_new_sale_price and new_sale_price == 0 and
                    product.name)):
                if not title:
                    title = _('Warning for product: %s') % product.name
                    ...
                    my_message = _("{}, and the new sale price.").format(
                        message)
                    message = my_message
            if title:
    ...
    def create(self, values):
        if ('generate_last_price_change_date' in values and
                values.get('generate_last_price_change_date', False)):
            values.update(
        ...
        template = super(ProductTemplate, self.with_context(
            product_created_from_template=True)).create(values)
    ...
    def write(self, values):
        ...
        if ('update_base_cost' not in self.env.context and len(keys) == 1 and
            ...
            values['standard_price'] = (
                values.get('standard_price') + self.extra_cost)
        if ('generate_last_price_change_date' in values and
                values.get('generate_last_price_change_date', False)):
            values.update(
                {'last_price_change_date': fields.Date.context_today(self),
                 'generate_last_price_change_date': False})
        if ('update_base_cost' in self.env.context and len(self) == 1 and
                'standard_price' in values):
            values['base_cost'] = values.get('standard_price')
            values['standard_price'] = (
                values.get('standard_price') + self.extra_cost)
        ...
        if 'new_extra_cost' in values:
            values['last_change_date_new_extra_cost'] = (
                fields.Date.context_today(self))
        if 'new_sale_price' in values:
            values['last_new_sale_price_change_date'] = (
                fields.Date.context_today(self))
        found = False
        if ('price_cost_changed' in values and
                values.get('price_cost_changed', False)):
    ...
    def _get_combination_info(self, combination=False, product_id=False,
        ...
        self.ensure_one()
        result = super(
            ProductTemplate, self)._get_combination_info(
...
#            result['price'] = imp
    ...
    def put_template_info_in_product(self):
        ...
        if (product.product_category_sale_price_id !=
                self.product_category_sale_price_id):
            vals["product_category_sale_price_id"] = (
                self.product_category_sale_price_id.id if
                self.product_category_sale_price_id else None)
        ...
        if (product.new_product_category_sale_price_id !=
                self.new_product_category_sale_price_id):
            vals["new_product_category_sale_price_id"] = (
                self.new_product_category_sale_price_id.id if
                self.new_product_category_sale_price_id else None)
        ...
        product.write(vals)
        if (product.last_new_sale_price_change_date !=
        ...
        if (product.last_change_date_new_extra_cost !=
...
class ProductProduct(models.Model):
    ...
    template_attributes_count = fields.Integer(
        string='Template attributes count', store=True,
