...
class RepairOrder(models.Model):
    ...
    task_state = fields.Selection(
        selection=[
            ("uninitiated", _("Task Uninitiated")),
    ...
    def _compute_only_read(self):
        for repair in self:
            if (
                repair.state in ("confirmed", "done", "2binvoiced")
                or repair.invoiced
                or (repair.state == "under_repair" and repair.finished_task)
            ):
    ...
    def action_invoice_create(self, group=False):
        result = super(
            RepairOrder, self.with_context(no_catch_notes=True)
        ).action_invoice_create(group=group)
        return result
    ...
    def action_cancel_validation(self):
        ...
        if (
            len(self) == 1
            and self.state == "2binvoiced"
            and self.invoice_method == "b4repair"
        ):
            found = True
        result = super().action_cancel_validation()
        for repair in self:
            if found:
