...
class AccountInvoice(models.Model):
    ...
    def _compute_expiration_dates(self):
        for invoice in self.filtered(
            lambda x: x.move_type == "out_invoice" and x.move_id
        ):
            lines = (
                invoice.move_id.mapped("line_ids")
                .filtered(lambda x: x.credit == 0 and x.debit > 0 and x.date_maturity)
    ...
    expiration_dates = fields.Char(
        ...
        compute="_compute_expiration_dates",
    )
    have_lots = fields.Boolean(
        string="Lots count",
        compute="_compute_have_lots",
    )
